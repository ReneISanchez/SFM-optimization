...
void MatchFeatures(const Mat& img_1, const Mat& img_1_orig, const Mat& img_2,
		const Mat& img_2_orig, const vector<KeyPoint>& imgpts1,
		const vector<KeyPoint>& imgpts2, const Mat& descriptors_1,
		const Mat& descriptors_2, vector<KeyPoint>& fullpts1,
		vector<KeyPoint>& fullpts2, int strategy, vector<DMatch>* matches) 
{
	//strategy
	bool use_features_for_matching = (strategy & STRATEGY_USE_FEATURE_MATCH)
			> 0;
	bool use_optical_flow_for_matching = (strategy & STRATEGY_USE_OPTICAL_FLOW)
			> 0;
...
//			desc.copyTo(descriptors_2);
		...
		for (unsigned int i = 0; i < matches->size(); i++) 
		{
			...
			if (existing_trainIdx.find((*matches)[i].trainIdx)
					== existing_trainIdx.end() && (*matches)[i].trainIdx >= 0
					&& (*matches)[i].trainIdx < (int) (keypoints_2.size())
					&& (*matches)[i].distance > 0.0
					&& (*matches)[i].distance < cutoff) 
			{
...
#endif
			Mat F = findFundamentalMat(pts1, pts2, FM_RANSAC, 0.1, 0.99,
					status);
		}
		cout << "Fundamental mat is keeping " << countNonZero(status) << " / "
				<< status.size() << endl;
			...
			if (use_optical_flow_for_matching) 
			{
				...
				for (int x = 0; x < img_1.cols; x++) 
				{
					for (int y = 0; y < img_1.rows; y++) 
					{
						...
						Mat_<double> moved = T
								* (Mat_<double>(3, 1) << x, y, 1);
						Point2f movedpt(moved(0), moved(1));
						flow_from_features(y, x) = Point2f(movedpt.x - x,
								movedpt.y - y);
#ifdef __SFM__DEBUG__
	...
	if (use_optical_flow_for_matching) 
	{
		...
		if (use_dense_optflow) {
			...
			{
				//coarse
				calcOpticalFlowFarneback(img_1, img_2, flow, 0.5, 5, 150, 60, 7,
						1.5, OPTFLOW_FARNEBACK_GAUSSIAN);
			}

			//refine
			calcOpticalFlowFarneback(img_1, img_2, flow, 0.5, 2, 40, 40, 5, 0.5,
					OPTFLOW_USE_INITIAL_FLOW);
			calcOpticalFlowFarneback(img_1, img_2, flow, 0.5, 0, 25, 40, 3,
					0.25, OPTFLOW_USE_INITIAL_FLOW);
...
//						imgpts2.push_back(KeyPoint(p1,1));
						good_matches_.push_back(
								DMatch(imgpts1.size() - 1, imgpts1.size() - 1,
										1.0));
		...
		else 
		{
			...
			cornerSubPix(img_1, corners, Size(15, 15), Size(-1, -1),
					TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 40,
							0.001));
			calcOpticalFlowPyrLK(img_1, img_2, corners, nextPts, status, err,
					Size(45, 45));
...
//					imgpts2.push_back(KeyPoint(nextPts[i],1));
	...
	else if (use_horiz_disparity) 
	{
		...
		Mat small_im1;
		resize(img_1_orig, small_im1, Size(), downscale, downscale);
		Mat small_im2;
		resize(img_2_orig, small_im2, Size(), downscale, downscale);
		...
		Mat outputflow;
		img_1_orig.copyTo(outputflow);
		Mat_<short> disp_orig_scale;
		resize(disp, disp_orig_scale, img_1.size());
